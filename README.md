...
If this fits your team, give it a star now and come back when you need a serious local context runtime.

## What It Solves / 它解决什么

很多团队已经有很多 agent，但没有一条统一、可信、可运维的上下文主链。  
Most teams already have many agents, but not one reliable, operable context backbone.
...
更详细的双语架构说明见 [ARCHITECTURE.md](docs/ARCHITECTURE.md)。
...
## Why Not MCP / Cloud Memory / 为什么不是别的路线
...
首先是产品，其次才是代码仓库。  
It is a product first, and a repo second.
...
不需要。默认完全本地。  
No. It runs fully local by default.

### 它是否必须接向量数据库或向量 API？

默认不需要。只有在你要做跨语义、低关键词重合、超长文本弱召回时，才值得评估可选向量层。  
No, not by default. Evaluate vectors only when weak semantic recall really matters.
...
- 当前版本：`0.6.1`
- 发布说明：[RELEASE_NOTES_0.6.1.md](docs/RELEASE_NOTES_0.6.1.md)
